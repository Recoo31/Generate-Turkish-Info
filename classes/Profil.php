...
class Profil
{
	public $isim, $soyisim, $tam_isim, $cinsiyet, $sehir, $adres, $meslek, $eposta, $kullanici_adi, $sifre, $dogum_tarihi, $yas, $telefon_numarasi, $ilgi_alani, $biyografi, $web_adresi;
	...
	private function kullanici_adi() {
		...
		$yap   = array('i', 'I', 'c', 'C', 'U', 'u', 'O', 'o', 's', 'S', 'g', 'G');

		$kullanici_adlari   = array();
		$kullanici_adlari[] = ucfirst(str_replace($bul, $yap, $this->tam_isim));
		$kullanici_adlari[] = ucfirst(ucfirst(str_replace($bul, $yap, $this->isim))).ucfirst(ucfirst(str_replace($bul, $yap, $this->soyisim)));
		$kullanici_adlari[] = ucfirst(ucfirst(str_replace($bul, $yap, $this->soyisim))).ucfirst(ucfirst(str_replace($bul, $yap, $this->isim)));
		$kullanici_adlari[] = ucfirst(str_replace($bul, $yap, $this->tam_isim)).mt_rand(00,999);
		$kullanici_adlari[] = ucfirst(str_replace($bul, $yap, $this->isim)).mt_rand(00,999);
		$kullanici_adlari[] = ucfirst(str_replace($bul, $yap, $this->soyisim)).mt_rand(00,999);
